...
use crate::{assert_called_once, kernel::{KERNEL, Kernel}, memory::{MemoryError, PhysicalAddress, MemoryRange}};
...
struct SimpleFrameAllocatorInner {
    ...
    kernel_prohibited_memory_ranges: [MemoryRange; Kernel::prohibited_mem_ranges_len()],
...
impl SimpleFrameAllocator {
    ...
    pub(in crate::memory::frames) const fn new() -> Self {
        SimpleFrameAllocator(Mutex::new(SimpleFrameAllocatorInner {
            ...
            kernel_prohibited_memory_ranges: [MemoryRange::empty(); Kernel::prohibited_mem_ranges_len()],
            initialized: false,
        }))
    }
